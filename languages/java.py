...
class JavaTask(CompilableTask):
  ...
  def __init__(self):
    ...
    self.execute_string = "java -Djava.security.manager -Xmx140m -Djava.security.policy={0}/java.policy Checker".format(config.PLAYGROUND_FILES_PATH)
    self.checker_file = "Checker.java"
    self.filename = "Solution.java"
    self.compile_strings = ["javac Solution.java", "javac Checker.java"]

  def set_solution_class_name(self, name):
    self.filename = name + ".java"
    self.compile_strings[0] = "javac " + self.filename
